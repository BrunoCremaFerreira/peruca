...
from domain.entities import DisambiguationCandidate, PendingMaintenanceFlow
...
class MaintenanceFlowService:
    ...
    def _focus_key(self, user_id: str) -> str:
        return f"{self._FOCUS_PREFIX}{user_id}"

    # ------------------------------------------------------------------ #
    # Persistence
    # ------------------------------------------------------------------ #
    async def set_pending(self, user_id: str, pending: PendingMaintenanceFlow) -> None:
    ...
    async def get_pending(self, user_id: str) -> Optional[PendingMaintenanceFlow]:
    ...
    async def set_focus(self, user_id: str, focus: dict) -> None:
        payload = dict(focus)
        payload["expires_at"] = time.time() + self.ttl_seconds
        await self.context_repository.set_key(
            self._focus_key(user_id), json.dumps(payload)
        )

    async def get_focus(self, user_id: str) -> Optional[dict]:
        raw = await self.context_repository.get_key(self._focus_key(user_id))
        if raw is None or raw == "None":
            return None
        try:
            data = json.loads(raw)
        ...
        if expires_at and expires_at < time.time():
            await self.clear_focus(user_id)
            return None
        return data
    ...
    def parse_slot_reply(
        self, pending: PendingMaintenanceFlow, message: str, vehicles=None
    ) -> SlotReplyResult:
    ...
    def _try_correction(
        self, message: str, pending: PendingMaintenanceFlow, expected: Optional[str]
