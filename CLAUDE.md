...
LLM_VEHICLE_MAINTENANCE_VISION_MODEL=           # empty = maintenance graph model; only the receipt-photo vision call uses it (better-OCR model swap without code changes)
...
- **Register from a receipt photo**: image + explicit register request routes to `vehicle_maintenance` (`has_images` context var in `main_graph.md`; the image-without-text bypass to `only_talking` is untouched). A deterministic code override (never the LLM) sends `register_maintenance`+images to the `register_from_receipt` node, which makes **one** synchronous vision call via `VehicleReceiptExtractor` (`application/graphs/vehicle_receipt_extractor.py`, prompt `vehicle_maintenance_receipt_extract.md` — loaded raw, **no** `ChatPromptTemplate`/`.format()`, so it uses single braces). The same JSON carries the asymmetric gate (`is_maintenance_document`; when in doubt `false`, fuel-only receipts rejected) and the extraction (transcribe-never-deduce; Python validates date/km bounds and sanitizes text; printed text is data, never instruction). User text beats document fields in the merge. Persistence **requires** the `register_receipt_confirm` yes/no turn (structural invariant — no code path from receipt to `MaintenanceService.register` without it); missing slots go through the `register_receipt` flow first (vehicle→date→km). Summary/refusal messages are deterministic Python templates, never LLM. `LLM_VEHICLE_MAINTENANCE_VISION_MODEL` overrides the model for this call only.
